class Flowfield {
  constructor(_resolution, _noiseRes, width, height) {
    ...
    this.increment = this.mapRange(this.noiseRes, 1, 1000, 0.00001, 0.05);
    // this.increment = this.noiseRes;
    // console.log(this.increment)
    // this.octaves = parseInt(this.mapRange(Math.random(), 0, 1, 0, 8));
    // this.falloff = parseFloat(
    //   this.mapRange(Math.random(), 0, 1, 0, 0.7)
    // ).toFixed(2);
    // noiseDetail(this.octaves, this.falloff);
    // // 4, 0.9 are good values!
    // this.seed = parseInt(this.mapRange(Math.random(), 0, 1, 0, 1000));
    // noiseSeed(this.seed);
    ...
    for (let y = 0; y < this.height; y += this.resolution) {
      ...
      for (let x = 0; x < this.width; x += this.resolution) {
        ...
        this.flowPoints.push({
          x: x,
          y: y,
          r: this.mapRange(noise.perlin2(this.xoff, this.yoff), -1, 1, 0, 1),
        });
      }
    }
  ...
  //   for (let y = 0; y < this.fieldHeight; y++) {
  //     yoff += this.increment;
  //     this.flowPoints[y + x * this.fieldHeight].r = this.mapRange(noise.perlin2(xoff, yoff), -1, 1, 0, 1);
  //   }
  // }
  // }
  ...
  //     textSize(14);
  //     textAlign(LEFT, TOP);
  //     text("Noise seed: " + this.seed, 10, 10);
  //     text("Octaves: " + this.octaves, 160, 10);
  //     text("Falloff: " + this.falloff, 310, 10);
  //   }
  // }
  ...
  getFlowPointByCanvasPosition(x, y) {
    let index =
      Math.floor(this.mapRange(x, 0, this.width, 0, this.xvals)) +
      Math.floor(this.mapRange(y, 0, this.height, 0, this.yvals)) * this.xvals;
    return index;
  }
}
